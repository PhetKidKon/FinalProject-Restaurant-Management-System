...
import { IngredientbaseStockTable } from "../../../content/DataTable";
import { AddIngrMenuUsed } from "../../../content/EditPopup";


const ISIngredientbasePage = ({}) => {
    ...
    const Fetchdata = async () => {
        try {
            const response = await axios.get("http://localhost:3001/api/menu/ingrbase/"+menuId);

            setData(response.data);
            setStockData(response.data.ingr_used);

            const ingrResponse = await axios.get("http://localhost:3001/api/ingr");
            setIngredientData(ingrResponse.data);
            ...
            <div className="bg-white rounded-lg pb-4 p-4 shadow h-[100vh]">
                    { !isLoading ? 
                            ...
                            <div className="flex flex-wrap px-4 space-x-5 items-center">
                                ...
                                <label className="text-2xl font-semibold px-5">สูตร Menu : {data.name}</label>
...

                                {
                                    showPopup && createPortal(
                                        <AddIngrMenuUsed trigger={showPopup} setTrigger={setShowPopup} data={data} setAlert={setAlert} setAlertMessage={setAlertMessage} fetchData={Fetchdata} ingr={ingredientData}></AddIngrMenuUsed>
                                        , document.body
                                    )
                                }
                            ...
                            <div className="flex-grow">
                                <IngredientbaseStockTable data={data} stockdata={filteredData} fetchdata={Fetchdata} setAlert={setAlert} setAlertMessage={setAlertMessage} role={"inventorysupervisor"} email={email}/>
                            </div>
...
export default ISIngredientbasePage;